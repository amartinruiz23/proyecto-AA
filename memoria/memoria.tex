...
Para el ajuste de Support Vector Machine vamos a considerar los parámetros C o  fuerza de regularización y kernel. Al no tener clasificación multiclase en este problema no tiene sentido aplicar estudio de varias de las variables. También estudiaremos el parámetro \texttt{max\_iter}.
\begin{itemize}
\item \underline{Kernel:} Consideraremos kernel Gaussiano como viene recomendado en el guión así como lineal. No elegimos kernel polinómico debido al estudio ya realizado sobre este tema en Regresión logística.\\
  ...
  Debido al éxito del modelo lineal estamos interesados en saber como se comportará un plano óptimo. Sin embargo, no descartamos que este modelo no sea lo suficientemente complejo como para estimar la variabilidad de los datos. Hay que considerar también dentro del contexto de la codificación realizada de los datos por qué el modelo lineal puede estar funcionando tan bien. En efecto, la existencia de variables dummy de existencia o no añaden mucha dimensionalidad permitiendo hacer a estos planos mucho más complejos (en particular, tenemos un plano de ¡55 dimeniones!), por lo cual este puede ser suficiente. Confiamos en que RBF consiga un mejor resultado de todos modos. En caso de que ambos consiguieran resultados similares escogeríamos Linear por el principio de la navaja de Ockham. Vemos en los resultados que:\\
...
Lineal & 0.85061 & 0.78141 \\
Gaussiano & 0.85061 & 0.78141  \\\hline
\end{tabular}
\caption{Resultados ajustes con variables polinómicas.}
