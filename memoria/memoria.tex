...
=======
Como último detalle de codificación de los datos, durante la parte de preprocesado realizamos la técnica de dummy variables. Esta permite entender las variables categóricas como variables numéricas. Para cada uno de los posibles valores de las variables categóricas se genera una nueva columna en la que se señala si el ejemplo en cuestión tiene o no ese valor.
>>>>>>> 6e4339eeddb5d4f15591e4a72e2a78e5db219b13
...
\begin{itemize}
	...
	\item \underline{Normalización de las variables}: realizaremos una normalización de los datos, para evitar que su escala afecte a la relevancia de estos. Para ello usaremos la función \texttt{StandardScaler}\cite{standardscaler} provista en la librería \texttt{sklearn}.
		\item \underline{Reducción de dimensionalidad}: Para esto vamos a realizar dos técnicas:
	
	\begin{itemize}
	\item[-] El conjunto de datos presenta una altisima cantidad de columnas (64) a pesar de la reducción indicada. Por ello vamos a intentar reducir aquellas que presenten poca o ningúna información haciendo un análisis por varianza, y eliminando aquellas columnas cuya varianza sea inferior a 0.1. Esto lo realizaremos con el algoritmo  \texttt{varianceThreshold}\cite{einstein}.
