...
Además, a la hora de validar la selección de hiperparámetros utilizaremos la técnica CrossValidation, para la cual diviremos el conjunto de training en 5 subconjuntos. Se utilizará para esta la función \texttt{cross\_validate}\cite{cv}.
...
\begin{itemize}
	...
	\item Normalización de las variables: realizaremos una normalización de los datos, para evitar que la escala de los datos afecte a la relevancia de estos. Para su uso usaremos la función \texttt{StandardScaler}\cite{standardscaler} provisto en la librería \texttt{sklearn}.
	\item Aumento de dimensionalidad: debido a la baja cantidad de variables, en contraposicición con la alta cantidad de ejemplos, nos deja considerar aumentos de la dimensionalidad de los datos sin miedo a que se genere un gran sobreajuste sobreajuste. 	
	\begin{itemize}
	\item Dummy Variables:
	\item Polynomial Freatures: 
\end{itemize}
